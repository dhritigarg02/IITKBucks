...
def Alias_or_pubKey():
    ...
    if answers['IDtype'] == 'Public Key':
        ...
        return {**answer, 'alias': None}
    else:
        answer = prompt(alias)
        return {'pubkey' : None, **answer}
...
def addAlias():    
    ...
    publickey = open(answers['publickey'], 'r').read()
    data = {
            'alias':answers['alias'], 
            'publicKey':publickey
            }
...
def getBalance():
    ...
    if answer['pubkey'] != None:
        pubkey = open(answer['pubkey'], 'r').read()
...
def Transfer():
    ...
    keys = prompt(keys)
    Txn['keys'] = keys
    pubkey = open(keys['pubkey'], 'r').read()
    json = {'publicKey': pubkey}
    r = requests.post(MY_URL+'/getUnusedOutputs', json = json)
    if r.status_code == 200:
        Txn['unusedOutputs'] = r.json()['unusedOutputs']
        balance = 0
        for output in Txn['unusedOutputs']:
            balance += int(output['amount'])
        Txn['balance'] = balance
        print("\nYour account balance is:", balance, '\n')
        ...
        if balance > 0:
            ...
            for i in range(int(answer['num_outputs'])):
                ...
                if answer['alias'] != None:
                    ...
                    if r.status_code == 200:
                        data = r.json()
                        outputs[i]['publicKey'] = data['publicKey'].encode()
                    else:
                ...
                total_amount += int(answer['amount'])
                outputs[i]['amount'] = int(answer['amount'])
            
            Txn['outputs'] = outputs
            answer = prompt(txnfees)
            total_amount += int(answer['txnfees'])
            Txn['txnfees'] = int(answer['txnfees'])
